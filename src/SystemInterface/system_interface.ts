...
export interface MMIODevice{
    read(addres: number): number;
    write(addres: number,value: number):void;
}
...
export class SystemInterface implements MMIODevice{
    ...
    read(address: number){
        if ((address & 0b11)!=0){
            throw new Error('Unaligned read from addres 0x${toHexString(address)}')
        }
    ...
    write(address: number, value: number){
        if ((address & 0b11)!=0){
            throw new Error('Unaligned write from addres 0x${toHexString(address)} value=0x${toHexString(value)}')
        }
